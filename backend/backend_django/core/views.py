...
class CrimeViewSet(viewsets.ModelViewSet):
  ...
  def retrieve(self, request, *args, **kwargs):
    ...
    # Check if this is retrieved by an analyst for the first time
    if request.user and request.user.role == 'analyst' and instance.citizen:
      has_reviewed = False
      if isinstance(instance.notes, list):
        for note in instance.notes:
          if "[System Alert] 'Analyst Review Started'" in note.get('note', ''):
            has_reviewed = True
            break
      if not has_reviewed:
        send_case_progression_email(instance, "Analyst Review Started")
        # Re-fetch instance after sending to include the new system note in response
        instance = self.get_object()
  ...
  def add_note(self, request, pk=None):
    ...
    # Check if the note is a request for evidence
...
class EvidenceViewSet(viewsets.ModelViewSet):
  ...
  def perform_create(self, serializer):
    ...
    if file_obj:
      evidence = serializer.save(file_path=f"/uploads/{file_obj.name}")
    else:
      evidence = serializer.save()

    # Citizen Notifications
    if evidence.linked_crime and evidence.linked_crime.citizen:
      send_case_progression_email(evidence.linked_crime, "Evidence Received")
